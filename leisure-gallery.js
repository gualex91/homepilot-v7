...
(function(){
 ...
 function clear(){window.hpLeisureEditor?.reset();equipment=[];tasks=[];owner='';message='';pending.clear();paint();}
 ...
 document.addEventListener('click',async e=>{
  ...
  try{
   ...
   else if(action==='edit-equipment')window.hpLeisureEditor?.open(x,owner);
   ...
   else if(action==='delete-equipment'){if(window.hpLeisureEditor)window.hpLeisureEditor.remove(x,owner,tasks.filter(t=>t.leisure_equipment_id===x.id).length);else await window.hpDeleteLeisure?.(x.id);}
  }catch{message='Cette action n’a pas pu être terminée. Réessaie.';status();}
  finally{pending.delete(key);b.disabled=false;}
 });
 function updated(record,userId){if(owner!==userId)return;equipment=equipment.map(x=>x.id===record.id?{...x,...record}:x);message='Fiche modifiée. Tes tâches sont conservées.';paint();}
 function deleted(id,userId){if(owner!==userId)return;equipment=equipment.filter(x=>x.id!==id);tasks=tasks.filter(t=>t.leisure_equipment_id!==id);message='Le loisir et ses tâches ont été supprimés.';paint();}
 window.hpLeisureGallery={render,loading,error,clear,updated,deleted};
})();
