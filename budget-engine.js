(function(root){
  ...
  function empty(){return {version:1,incomes:[],bills:[],envelopes:[],provisions:[],reviewed:false,cash:{balance:null,asOf:null,todaySettled:false},emergencySavings:null}}
  function validate(input){
    ...
    for(const key of ['incomes','bills','envelopes','provisions']){
      if(!Array.isArray(input[key])||input[key].length>100)fail('Trop de lignes dans le budget.');
      result[key]=input[key].map(x=>{
  ...
  function analyze(plan,entries,month,today,entriesComplete=true){
    const start=month+'-01',end=monthEnd(month),calendar=events(plan,start,end),totals={income:0,bills:0,flexible:0,provisions:0,essential:0};
    ...
    const projectedMargin=totals.income-totals.bills-totals.flexible-totals.provisions;
    ...
    let cash=null;
    if(plan.reviewed&&plan.cash.balance!==null&&plan.cash.asOf===today&&plan.cash.todaySettled&&nextPay&&!provisions.some(x=>x.overdue)){
      const scheduled=nextEvents.filter(x=>x.kind!=='income'&&x.date<=nextPay.date);
      const bills=scheduled.reduce((n,x)=>n+(x.kind==='annual'?x.unfunded:x.amount),0);
      // Daily envelope allowance, including the payday before its deposit.
      const dueIds=new Set(scheduled.filter(x=>x.kind==='annual').map(x=>x.id));
      const reserveMonthly=provisions.filter(x=>!dueIds.has(x.id)).reduce((n,x)=>n+x.recommended,0);
      let flexible=0,reserve=0;for(let d=addDays(today,1);d<=nextPay.date;d=addDays(d,1)){const divisor=Number(monthEnd(d.slice(0,7)).slice(-2));flexible+=totals.flexible/divisor;reserve+=reserveMonthly/divisor}
    ...
    if(!cash&&plan.reviewed)actions.push({title:'Actualiser mon disponible',body:'Un solde vérifié aujourd’hui, une prochaine rentrée d’argent positive et des échéances annuelles à jour sont nécessaires pour cette estimation.'});
    ...
    return {month,totals,actual:entriesComplete?actual:null,projectedMargin,complete:plan.reviewed&&plan.incomes.length>0,categories:[...categories.values()],provisions,calendar,cash,emergencyMonths:plan.emergencySavings!==null&&totals.essential>0?cents(plan.emergencySavings)/totals.essential:null,actions:actions.slice(0,3)};
  }
  root.hpBudgetEngine={empty,validate,occurrences,events,analyze,validDate,monthEnd,addDays,cents};
})(globalThis);
