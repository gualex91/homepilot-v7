...
import {paymentsFor,paymentEngine as P} from '../lib/asset-payments.js';
...
export default async function handler(req,res){
  ...
  try{
    ...
    async function success(result){
      const payments=await paymentsFor(query,user.id);
      return res.status(200).json({...result,config:result.config||payments.length?P.merge(result.config||engine.empty(),payments):null,asset_payments:payments});
    }
    const current=present((await query('budget_plans',[...own,['select','config,maintenance_projects,revision,updated_at']]))[0]);
    if(req.method==='PUT'){
      let config;
      try{
        const input=P.cleanPlan(req.body?.config);
      ...
      if(current?.revision===id&&stable(current.config)===stable(config))return await success({ok:true,...current});
      ...
      if(!rows.length){
        ...
        if(latest?.revision===id&&stable(latest.config)===stable(config))return await success({ok:true,config:latest.config,revision:latest.revision,updated_at:latest.updated_at});
        return res.status(409).json({error:'Une autre sauvegarde a été effectuée. Recharge le budget.'});
      }
      return await success({ok:true,...present(rows[0])});
    }
    ...
    return await success({ok:true,config:current?.config||null,revision:current?.revision||null,updated_at:current?.updated_at||null,entries,entries_complete:complete,legacy});
  }catch(error){
