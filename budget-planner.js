(function(){
  ...
  let assetPayments=[],paymentVersion=0,retryNeeded=false;
  ...
  function reset(){assetPayments=[];paymentVersion++;retryNeeded=false;closeScenario();picker=null;simulationAmount=0;epoch++;owner=null;ready=false;dirty=false;revision=null;loading=false;saving=false;lastLoadedMonth=null;plan=E.empty();draft=E.empty();entries=[];legacy=null;lastResult=null;ensure();render();status('Connecte-toi pour retrouver ton budget privé.');}
  ...
  async function onClick(event){
    ...
    if(action==='save')return save();if(action==='retry'||action==='reload')return load();if(action==='replace')return load({force:true});if(action==='add')return add(b.dataset.list);
