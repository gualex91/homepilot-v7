(function(){
  ...
  const alpha=(a,b)=>C.alphabetical(a,b);
  ...
  let assetPayments=[],paymentVersion=0,retryNeeded=false,activeExpense=null;
  ...
  function status(message,error=false){
    ...
    for(const id of ['hpFinanceStatus','hpFinanceSaveStatus','hpExpenseSaveStatus']){const el=$(id);if(el){el.textContent=message;el.setAttribute('role',error?'alert':'status');el.classList.toggle?.('finance-negative',error)}}
    updateLoadControls();
  }
  function updateLoadControls(){document.querySelectorAll('#hpBudgetPlanner [data-action="retry"]').forEach(el=>{el.hidden=!retryNeeded;el.disabled=loading||saving})}
  ...
  function ensure(){
    ...
    box.innerHTML=`<div class="finance-heading"><div><h3>Mes finances</h3><div class="finance-note">Moyenne mensuelle : chaque semaine × 52 ÷ 12; aux deux semaines × 26 ÷ 12. Les opérations et le calendrier conservent les montants réels.</div></div></div><div class="finance-entry-shortcuts" aria-label="Ajouter une opération"><button type="button" data-action="entry" data-entry-type="income">+ Ajouter un revenu</button><button type="button" data-action="entry" data-entry-type="expense">+ Ajouter une dépense</button><p class="finance-note">Un montant reçu ou payé, à enregistrer en quelques champs.</p></div><div class="finance-controls"><button type="button" class="alt" data-action="previous" aria-label="Mois précédent">‹</button><input id="hpFinanceMonth" type="month" value="${month}" aria-label="Mois du budget"><button type="button" class="alt" data-action="next" aria-label="Mois suivant">›</button></div><section id="hpFinanceMoneyHeadline" aria-label="Mon budget prévu"></section><div class="finance-tabs" role="tablist" aria-label="Mon budget"><button type="button" data-view="overview" role="tab" id="hpFinanceTabOverview" aria-controls="hpFinanceOverview">Mon bilan</button><button type="button" data-view="plan" role="tab" id="hpFinanceTabPlan" aria-controls="hpFinanceEditor">Mes montants</button><button type="button" data-view="operations" role="tab" id="hpFinanceTabOperations" aria-controls="hpFinanceOperations">Mes opérations</button><button type="button" data-view="calendar" role="tab" id="hpFinanceTabCalendar" aria-controls="hpFinanceCalendar">Mes dates</button></div><p id="hpFinanceStatus" class="finance-note" role="status" aria-live="polite">Chargement du budget…</p><details class="finance-load-options"><summary>Actualiser les données</summary><button type="button" class="alt" data-action="reload">Actualiser sans perdre ma saisie</button><button type="button" class="alt" data-action="replace">Revenir à la version enregistrée</button></details><button type="button" class="alt" data-action="retry" hidden>Réessayer le chargement</button><section id="hpFinanceScenario" class="finance-block finance-scenario" aria-labelledby="hpFinanceScenarioTitle" hidden></section><div id="hpFinanceOverview" role="tabpanel" aria-labelledby="hpFinanceTabOverview"></div><div id="hpFinanceEditor" role="tabpanel" aria-labelledby="hpFinanceTabPlan" hidden></div><section id="hpFinanceOperations" role="tabpanel" aria-labelledby="hpFinanceTabOperations" hidden><h3>Mes opérations</h3><p class="finance-note">Ce que tu as réellement reçu ou payé ce mois-ci. Les montants prévus ne créent pas de paiement automatique.</p><button type="button" data-action="entry">+ Noter un revenu ou un paiement</button></section><div id="hpFinanceCalendar" role="tabpanel" aria-labelledby="hpFinanceTabCalendar" hidden></div>`;
    ...
    box.addEventListener('keydown',event=>{const tab=event.target.closest('[role=tab]');if(!tab||!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();const tabs=[...box.querySelectorAll('[role=tab]')];let i=tabs.indexOf(tab);i=event.key==='Home'?0:event.key==='End'?tabs.length-1:(i+(event.key==='ArrowRight'?1:tabs.length-1))%tabs.length;setView(tabs[i].dataset.view);tabs[i].focus()});
    setView(view);
  }
  function reset(){activeExpense=null;assetPayments=[];paymentVersion++;retryNeeded=false;closeScenario();picker=null;simulationAmount=0;epoch++;owner=null;ready=false;dirty=false;revision=null;loading=false;saving=false;lastLoadedMonth=null;plan=E.empty();draft=E.empty();entries=[];legacy=null;lastResult=null;ensure();render();status('Connecte-toi pour retrouver ton budget privé.');}
  ...
  async function api(method,body,requestedMonth=month){const session=await identity();if(owner&&session.user.id!==owner)throw new Error('La session a changé. Recharge le budget.');const r=await fetch('/api/budget-plan?month='+requestedMonth,{method,headers:{Authorization:'Bearer '+session.access_token,'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined,cache:'no-store',signal:AbortSignal.timeout(15000)});let data;try{data=await r.json()}catch{throw new Error('La réponse du serveur est incomplète. Réessaie dans un instant.')}if(!r.ok)throw new Error(data.error||'Budget indisponible.');return data}
  async function load({force=false}={}){
    ensure();if(loading||saving)return;
    // Take the lock before the asynchronous session read: auth and navigation
    // can request a refresh at the same time.
    loading=true;let stamp=epoch;const requestedMonth=month;
    try{
      const session=await identity();if(stamp!==epoch)return;
      if(owner!==session.user.id){reset();owner=session.user.id;stamp=epoch;loading=true}
      if(force&&dirty&&!confirm('Remplacer tes montants par la version enregistrée effacera tes modifications non enregistrées. Continuer?'))return;
      status('Chargement…');const requestedPayments=paymentVersion;
      const data=await api('GET',undefined,requestedMonth);if(stamp!==epoch||requestedMonth!==month)return;
      ...
      entries=data.entries||[];entriesComplete=data.entries_complete!==false;legacy=data.legacy;ready=true;retryNeeded=false;lastLoadedMonth=month;
      // Refresh the analysis without replacing inputs being edited. Keep the
      // original revision while dirty; server-side CAS still rejects conflicts.
      render(!dirty);status(dirty?'Données actualisées. Tes modifications sont conservées et restent à enregistrer.':data.config?'Ton budget est enregistré.':'Commence par ajouter tes revenus et tes dépenses.');
    }catch(error){if(stamp===epoch){retryNeeded=true;entriesComplete=false;const message=error.name==='TimeoutError'?'Le chargement prend trop de temps.':error.message;status(message+(ready?' Tes saisies sont conservées. Tu peux continuer, enregistrer ou réessayer.':' Réessaie le chargement avant de saisir tes montants.'),true);render(false)}}
    finally{if(stamp===epoch){loading=false;updateLoadControls();updateScenario();if(requestedMonth!==month)load()}}
  }
  ...
  function renderMoneyHeadline(){
    const host=$('hpFinanceMoneyHeadline');if(!host)return;
    ...
    const hasAmounts=draft.incomes.length+draft.bills.length+draft.envelopes.length+draft.provisions.length+draft.projects.length>0;
    if(!hasAmounts){host.innerHTML='<div class="finance-start"><h3>Combien te reste-t-il chaque mois?</h3><p>Commence par tes revenus, puis ajoute tes dépenses et ton épargne.</p><button type="button" data-action="choose" data-kind="income">+ Ajouter mon premier revenu</button></div>';return}
    const note=complete?'Après tes dépenses et les sommes prévues à mettre de côté.':canEstimate?'Estimation à vérifier : calculée avec tes montants saisis, même avant la confirmation du budget.':'Ajoute un revenu dans « Mes montants ». Les revenus notés dans « Mes opérations » ne sont pas ajoutés automatiquement au budget prévu.';
    host.innerHTML=`<p class="finance-money-label">Budget prévu · ${esc(period)} <span class="finance-budget-state">${dirty?'Modifications non enregistrées':complete?'Budget vérifié':canEstimate?'Estimation à vérifier':'Revenus prévus manquants'}</span></p><div class="finance-grid finance-balance">${tile(canEstimate?(r.projectedMargin<0?(complete?'Il manque ce mois-ci':'Manque estimé ce mois-ci'):(complete?'Reste prévu ce mois-ci':'Reste estimé ce mois-ci')):'Ajoute tes revenus prévus',canEstimate?money(Math.abs(r.projectedMargin)):'—',note,true,canEstimate&&r.projectedMargin<0)}</div><dl class="finance-equation"><div><dt>Revenus prévus</dt><dd>${money(r.totals.income)}</dd></div><div><dt>Solde estimé reporté du mois précédent</dt><dd>${money(r.carryIn||0)}</dd></div><div><dt>− Dépenses prévues</dt><dd>${money(expenses)}</dd></div><div><dt>− Épargne et réserves</dt><dd>${money(reserve)}</dd></div></dl><p class="finance-note">${complete?'Solde reporté + revenus − dépenses − épargne et réserves = reste prévu.':'Totaux des lignes saisies; ton budget est encore à vérifier.'} Le report est une estimation cumulative selon ton budget prévu, à partir de ${esc(r.rolloverStartMonth||month)}. Ce n’est pas le solde de ton compte bancaire.</p>${!complete?`<button type="button" class="alt" data-action="review-budget">${draft.incomes.length?'Vérifier mon budget':'Ajouter mes revenus'}</button>`:''}`;
  }
  function renderOverview(){
    renderHomeBudget();renderContributionShortcuts();renderMoneyHeadline();
    ...
    host.innerHTML=`${dirty?'<p class="finance-warning">Aperçu de tes modifications non enregistrées. <button type="button" class="alt" data-view="plan">Vérifier et enregistrer</button></p>':''}${!complete?`<p class="finance-note">${draft.incomes.length?'Le reste estimé se met à jour avec tes montants. Tu peux vérifier ton budget à ton rythme.':'Ajoute tes revenus prévus pour estimer ce qu’il reste.'}</p>`:''}<div class="finance-quick-actions"><button type="button" data-view="plan">Modifier mon budget prévu</button><button type="button" class="alt" data-action="entry">Noter un revenu ou un paiement</button></div><details class="finance-block"><summary>Comprendre mes chiffres</summary>${complete?`<p>${info.committedPercent===null?'Aucun revenu positif prévu pour calculer une proportion.':`Sur 100 $ de revenus, <b>${info.committedPercent.toLocaleString('fr-CA',{maximumFractionDigits:1})} $</b> sont affectés aux dépenses et mises de côté.`}</p>`:''}${complete&&info.allocations.length?`<div class="finance-action"><b>Ton poste principal : ${esc(info.allocations[0].category)}</b><p>${money(info.allocations[0].amount)}, soit ${(info.allocations[0].amount/info.outflow*100).toLocaleString('fr-CA',{maximumFractionDigits:1})} % des sorties prévues. Ouvre « Où va mon argent? » pour voir les autres postes.</p></div>`:''}${r.actions.slice(0,2).map(a=>`<div class="finance-action"><b>${esc(a.title)}</b><p>${esc(a.body)}</p></div>`).join('')}</details><details class="finance-analysis"><summary>Où va mon argent?</summary><p class="finance-note">Répartition de toutes les sorties prévues, réserves comprises. Les catégories déjà enregistrées sont conservées.</p>${info.allocations.length?info.allocations.map(x=>`<div class="finance-allocation"><div class="finance-row-head"><span>${esc(x.category)}</span><b>${money(x.amount)}</b></div><meter min="0" max="${info.outflow}" value="${x.amount}" aria-label="${esc(x.category)} : ${money(x.amount)}"></meter><span class="finance-note">${(x.amount/info.outflow*100).toLocaleString('fr-CA',{maximumFractionDigits:1})} % des sorties prévues</span></div>`).join(''):'<p>Ajoute tes dépenses pour voir leur répartition.</p>'}${info.debts?`<p class="finance-note">${money(info.debts)} de remboursements de dettes identifiés ce mois. Vérifie les anciens postes classés ailleurs : ils restent inclus dans leur catégorie.</p>`:''}</details><details class="finance-analysis"><summary>Et si je changeais une habitude?</summary><p>Essaie une réduction de tes dépenses courantes marquées non essentielles.</p><p class="finance-note">Base déclarée : ${money(info.adjustable)}/mois. Tu choisis les dépenses essentielles dans les options de chaque ligne.</p><label for="hpFinanceSimulation">Réduction envisagée par mois ($)</label><input id="hpFinanceSimulation" data-simulation="amount" type="number" inputmode="decimal" min="0" max="${info.adjustable/100}" step="0.01" value="${simulationAmount}" ${!complete||!info.adjustable?'disabled':''}><div id="hpFinanceSimulationResult" aria-live="polite"></div><p class="finance-note">Simulation seulement : aucune dépense modifiée, aucun rendement supposé. Le résultat sur un an suppose la même réduction pendant 12 mois.</p></details><details class="finance-analysis"><summary>Mes dépenses enregistrées face au budget</summary><p class="finance-note">Montants saisis, hors dates futures. Aucun compte bancaire connecté; vérifie avec tes relevés.</p>${r.actual?`<p>Revenus enregistrés : <b>${money(r.actual.income)}</b> · Dépenses enregistrées : <b>${money(r.actual.expense)}</b></p>${comparison(r)}`:'<p role="alert">Les opérations n’ont pas toutes été chargées. Aucun total partiel n’est présenté.</p>'}<button type="button" class="alt" data-action="entry">Noter un revenu ou un paiement</button></details><details class="finance-analysis"><summary>Avant ma prochaine rentrée d’argent</summary>${r.cash?`<div class="finance-stat primary"><span>Disponible estimé jusqu’au ${dateLabel(r.cash.nextPay)}, avant le versement</span><strong class="${r.cash.available<0?'finance-negative':''}">${money(r.cash.available)}</strong></div><p class="finance-note">Solde déclaré ${money(r.cash.balance)} − paiements ${money(r.cash.bills)} − dépenses courantes estimées ${money(r.cash.flexible)} − réserves ${money(r.cash.reserve)}.</p>`:'<p>Ajoute un solde vérifié aujourd’hui et tes dates de revenus pour calculer cette estimation.</p><button type="button" class="alt" data-action="cash">Ajouter mon solde</button>'}<p class="finance-note">Calcul à partir de demain. Les paiements d’aujourd’hui doivent déjà être déduits du solde. Les paiements du jour de paie sont déduits avant le versement. Vérifie les dates si elles tombent un jour férié.</p></details><details class="finance-analysis"><summary>Mon coussin et mes projets</summary><h4>Mon coussin de sécurité</h4><p>${r.emergencyMonths!==null&&complete?`L’épargne de secours déclarée couvre environ <b>${r.emergencyMonths.toLocaleString('fr-CA',{maximumFractionDigits:1})} mois</b> de dépenses marquées essentielles.`:'Ajoute ton épargne de secours et vérifie les dépenses essentielles pour estimer la durée couverte.'}</p><p class="finance-note">Cette durée ne détermine pas tes besoins d’assurance.</p>${projectCards(r)}</details><p class="finance-note">Pour préparer tes chiffres : <a href="https://www.canada.ca/fr/agence-consommation-matiere-financiere/services/faire-budget.html" target="_blank" rel="noopener">guide budgétaire de l’Agence de la consommation en matière financière du Canada</a>.</p>${advisorCard(r,info)}`;
    renderSimulation();
  }
  function advisorCard(r,info){
    return `<div class="finance-advisor finance-advisor-compact"><h4>Besoin d’aide avec ces chiffres?</h4><p>Prépare tes prochaines étapes avec ton conseiller Nuvabri.</p><button type="button" data-action="advisor">Parler à mon conseiller</button><details><summary>Préparer ma rencontre</summary><ul>${info.questions.slice(0,2).map(q=>`<li>${esc(q)}</li>`).join('')}</ul><button type="button" class="alt" data-action="summary">Préparer mon résumé</button><p class="finance-note">Guillaume-Alexandre Tremblay · Conseiller en sécurité financière. Tu choisis les informations à partager.</p><div id="hpFinanceSummary" hidden><label for="hpFinanceSummaryText">Mon résumé à relire avant de le partager</label><textarea id="hpFinanceSummaryText" readonly></textarea><button type="button" class="alt" data-action="copy">Copier mon résumé</button></div></details></div>`;
  }
  ...
  function savingsField(key,x,i){
    if(!['bills','envelopes'].includes(key)||!I.savingsCategory(x.category))return '';
    return `${input(key+'.'+i+'.accountBalance','Montant actuel de ce compte d’épargne ($)',x.accountBalance,'number','min="0"')}${input(key+'.'+i+'.accountBalanceAsOf','Solde vérifié le',x.accountBalanceAsOf||'','date')}<p class="finance-note">Ce solde inclut les versements déjà effectués à cette date. Seules les cotisations suivantes s’ajoutent à la projection.</p>`;
  }
  ...
  function renderSavingsAnnual(){
    ...
    const accounts=I.annualSavings(draft),monthly=I.monthlySavings(draft,today());host.hidden=!accounts.length&&!monthly.length;
    if(!accounts.length&&!monthly.length){host.innerHTML='';return}
    ...
    host.innerHTML=`<h4>Mon épargne après 12 mois</h4><p class="finance-note">Projection au rythme choisi, sans rendement. Aucun versement automatique.</p><div class="finance-saving-grid">${accounts.map(x=>`<div class="finance-saving-account"><h5>${x.category}${x.accounts>1?' · '+x.accounts+' lignes':''}</h5><span>Cotisations estimées sur un an</span><strong>${amount(x.contributions)}</strong><dl><div><dt>Solde de départ</dt><dd>${x.startingBalance===null?'Non renseigné':money(x.startingBalance)}</dd></div><div><dt>Solde estimé après un an</dt><dd>${x.projectedBalance===null?'À compléter':money(x.projectedBalance)}</dd></div></dl>${x.startingBalance===null?'<p class="finance-note">Ajoute le solde de départ dans chaque ligne pour estimer le total accumulé.</p>':''}</div>`).join('')}</div><details><summary>Comment cette estimation est calculée</summary><p class="finance-note">52 cotisations pour une fréquence hebdomadaire, 26 aux deux semaines, 24 deux fois par mois ou 12 mensuelles. Un versement unique est compté une seule fois. La moyenne mensuelle utilise 52 ÷ 12 ou 26 ÷ 12. Le calendrier conserve les dates réelles des cotisations.</p><p class="finance-note">Solde estimé = solde de départ + cotisations. Sans rendement, frais, retraits ni effet fiscal. Projection sur 12 mois complets, pas le reste de l’année civile. Les cotisations sont déjà incluses dans « Épargne et réserves ».</p></details>${legacy?'<p class="finance-warning">Un ancien poste regroupe REER et CELI. Répartis son montant dans les nouvelles lignes et retire le poste groupé pour éviter de compter deux fois la même épargne.</p>':''}`;
    host.innerHTML+=monthly.map(account=>`<div class="finance-saving-account"><h5>${esc(account.label)}</h5><span>Solde estimé aujourd’hui</span><strong>${amount(account.currentEstimate)}</strong><p class="finance-note">${account.asOf?'À partir du solde confirmé le '+dateLabel(account.asOf)+'. Estimation selon les versements prévus, sans rendement ni retrait.':'Précise la date du solde dans cette ligne d’épargne pour calculer son évolution.'}</p><details><summary>Voir la projection mensuelle moyenne</summary>${account.points.map(point=>`<div class="finance-row"><span>${new Intl.DateTimeFormat('fr-CA',{month:'long',year:'numeric'}).format(new Date(point.date+'T12:00:00'))}</span><b>${amount(point.balance)}</b></div>`).join('')}</details></div>`).join('');
  }
  function editorRow(key,x,i){
    ...
    const categories=[...new Set([...C.categoriesFor(income?'income':'expense'),x.category])].sort(alpha).map(c=>[c,c]);
  ...
  function pickerHTML(){
    if(!picker)return '';
    const list=C.templates.filter(t=>picker==='income'?t.key==='incomes':t.key!=='incomes').sort((a,b)=>alpha(a.label,b.label));
    const families=[...new Set(list.map(t=>t.family))].sort(alpha);
  ...
  const EXPENSE_SUGGESTIONS=[['unexpected','Imprévu'],['rent','Loyer'],['mortgage','Hypothèque'],['hydro','Électricité / chauffage'],['phone','Téléphone'],['internet','Internet'],['subscriptions','Télé / abonnements'],['home-insurance','Assurance habitation'],['car-insurance','Assurance automobile'],['life-insurance','Assurance vie / invalidité'],['food','Épicerie'],['fuel','Essence / recharge'],['daycare','Garderie'],['rrsp','REER'],['tfsa','CELI']];
  ...
  function expenseSuggestions(kind='expense'){
    const savings=kind==='savings',choices=EXPENSE_SUGGESTIONS.filter(([id])=>I.savingsCategory(C.templates.find(t=>t.id===id).category)===savings).sort((a,b)=>alpha(a[1],b[1]));
    const editing=choices.some(([id])=>activeExpense?.template===id),hasRows=draft.bills.concat(draft.envelopes).some(x=>I.savingsCategory(x.category)===savings);
    return `<details id="${savings?'hpSavingsChoices':'hpExpenseChoices'}" class="finance-suggestions" ${editing||!hasRows?'open':''}><summary>${savings?'+ Ajouter un CELI ou un REER':'+ Choisir une dépense'}</summary><p class="finance-note">Choisis un poste, puis inscris ton montant.</p><div class="finance-suggestion-grid">${choices.map(([id,label])=>{const t=C.templates.find(x=>x.id===id),index=existingSuggestion(t),active=activeExpense?.template===id,draftIndex=active?draft[t.key].findIndex(x=>x.id===activeExpense.rowId):-1,opened=active&&draftIndex>=0;return `<div class="finance-suggestion-item ${opened?'is-editing':''}"><button type="button" class="alt finance-suggestion" data-action="suggest-expense" data-template="${id}" aria-expanded="${opened}"><span>${esc(label)}</span><small>${opened?'Saisie en cours':index>=0?'Déjà ajoutée · modifier':'+ Ajouter un montant'}</small></button>${opened?`${editorRow(t.key,draft[t.key][draftIndex],draftIndex)}<p id="hpExpenseSaveStatus" class="finance-note" role="status" aria-live="polite"></p><button type="button" data-action="save-continue">Enregistrer et continuer</button>`:''}</div>`}).join('')}</div></details>`;
  }
  function openExpenseSuggestion(id){
    ...
    activeExpense={template:id,rowId:draft[t.key][index].id};renderEditor();focusRow(t.key,index);status('Cette dépense est déjà prévue. Tu peux ajuster son montant.');
  }
  function renderEditor(){
    const host=$('hpFinanceEditor');if(!host)return;if(!ready){host.innerHTML='<p>Charge ton budget avant de le modifier.</p><button type="button" data-action="retry">Réessayer le chargement</button>';return}
    const rows=(key,filter=()=>true)=>draft[key].map((x,i)=>({x,i})).sort((a,b)=>alpha(a.x.label,b.x.label)).map(({x,i})=>activeExpense?.rowId===x.id||!filter(x)?'':editorRow(key,x,i)).join('');
    const expense=x=>!I.savingsCategory(x.category),savingRow=x=>I.savingsCategory(x.category);
    host.innerHTML=`<p class="finance-note">Prévois tes revenus, tes dépenses et ton épargne. Pour chaque montant, choisis la fréquence. Utilise ta part des dépenses du foyer.</p>${!revision&&legacy&&(legacy.targets.length||legacy.bills.length)?'<div class="finance-warning">Tu as déjà des montants enregistrés. <button type="button" class="alt" data-action="import">Reprendre mes montants existants</button><p>Vérifie les doublons avant de sauvegarder.</p></div>':''}${pickerHTML()}<section class="finance-block"><div class="finance-section-title"><span class="finance-step">1</span><h4>Mes revenus</h4></div><p class="finance-note">Salaire, allocations, revenus autonomes, retraite…</p>${rows('incomes')||'<p>Aucun revenu ajouté pour le moment.</p>'}<button type="button" class="alt" data-action="choose" data-kind="income">+ Ajouter un revenu</button></section><section class="finance-block"><div class="finance-section-title"><span class="finance-step">2</span><h4>Mes dépenses</h4></div>${rows('bills',expense)}${rows('envelopes',expense)}${expenseSuggestions()}${!draft.bills.length&&!draft.envelopes.length?'<p>Commence par ton logement, ton épicerie et ton transport.</p>':''}<button type="button" class="alt" data-action="choose" data-kind="expense">+ Autre dépense ou épargne</button></section><section class="finance-block"><div class="finance-section-title"><span class="finance-step">3</span><h4>Mon épargne</h4></div><p class="finance-note">CELI, REER et autres mises de côté. Ces montants sont déduits une seule fois dans ton budget.</p>${rows('bills',savingRow)}${rows('envelopes',savingRow)}${expenseSuggestions('savings')}<div id="hpFinanceSavingsAnnual" class="finance-saving-summary" aria-live="polite" hidden></div></section><details id="hpFinanceAnnual"><summary>Dépenses à préparer dans l’année · ${draft.provisions.length}</summary><p class="finance-note">Taxes, permis, vacances… Nuvabri calcule une réserve mensuelle. N’ajoute pas le même paiement dans tes dépenses habituelles.</p>${rows('provisions')}<button type="button" class="alt" data-action="choose" data-kind="expense">+ Choisir une dépense</button><button type="button" class="alt" data-action="add" data-list="provisions">Autre dépense annuelle</button></details><details><summary>Entretiens et remplacements · ${draft.projects.length}</summary><p class="finance-note">Pour un coût ponctuel, avec une date et un montant à préparer.</p>${rows('projects')}<button type="button" class="alt" data-action="add" data-list="projects">+ Prévoir un projet</button></details><details id="hpFinanceCash"><summary>Mon solde et mon coussin — facultatif</summary><p class="finance-note">Montant disponible pour payer, sans l’argent déjà réservé. Vérifie le solde à la date indiquée.</p>${input('cash.balance','Solde disponible aujourd’hui ($)',draft.cash.balance,'number')}${input('cash.asOf','Date de vérification',draft.cash.asOf,'date')}${checkbox('cash.todaySettled','Les paiements de cette journée sont déjà déduits.',draft.cash.todaySettled)}${input('emergencySavings','Épargne disponible pour les imprévus ($)',draft.emergencySavings,'number','min="0"')}<p class="finance-note">Laisse vide si tu ne connais pas ce montant. Une limite de crédit n’est pas de l’épargne.</p></details><details><summary>Éviter de compter deux fois la même somme</summary><p class="finance-note">Si tu as inscrit tes achats par carte, ne les ajoute pas au remboursement de la carte. Indique seulement le remboursement d’une ancienne dette en plus. Un virement entre comptes n’est pas un revenu. Les prêts, rappels et objectifs des outils détaillés ne sont pas ajoutés automatiquement à ce plan.</p></details><div id="hpFinanceReview" class="finance-review"><h4>Dernière étape : vérifier mon budget</h4>${checkbox('reviewed','J’ai ajouté tous mes revenus, mes dépenses et mon épargne. Mon budget est prêt.',draft.reviewed)}</div><div class="finance-savebar"><p id="hpFinanceSaveStatus" class="finance-note" tabindex="-1" role="status" aria-live="polite"></p><button type="button" class="alt" data-action="retry" ${retryNeeded?'':'hidden'}>Réessayer sans perdre ma saisie</button><button type="button" class="alt" data-action="save-continue" ${saving?'disabled':''}>Enregistrer et continuer</button><button type="button" data-action="save" ${saving?'disabled':''}>${saving?'Enregistrement…':'Enregistrer et voir mon bilan'}</button><div class="finance-note" id="hpFinanceDirty">${dirty?'Modifications non enregistrées.':'Budget personnel enregistré dans ton compte.'}</div></div>`;
  ...
  function addTemplate(id){
    ...
    draft[key].push(row);activeExpense=EXPENSE_SUGGESTIONS.some(x=>x[0]===id)?{template:id,rowId:row.id}:null;dirty=true;draft.reviewed=false;render();setView('plan');
  ...
  function setView(next){view=next;if($('hpFinanceMoneyHeadline'))$('hpFinanceMoneyHeadline').hidden=view==='operations';if($('budget'))$('budget').dataset.financeView=view;if($('hpBudgetTools'))$('hpBudgetTools').hidden=view!=='overview';const controls=document.querySelector('#hpBudgetPlanner .finance-controls');if(controls)controls.hidden=view==='operations';const map={overview:'hpFinanceOverview',plan:'hpFinanceEditor',operations:'hpFinanceOperations',calendar:'hpFinanceCalendar'};for(const [key,id] of Object.entries(map))if($(id))$(id).hidden=key!==view;document.querySelectorAll('#hpBudgetPlanner [data-view]').forEach(b=>{if(b.getAttribute('role')==='tab'){b.setAttribute('aria-selected',String(b.dataset.view===view));b.tabIndex=b.dataset.view===view?0:-1}})}
  window.hpSetFinanceView=next=>{if(['overview','plan','operations','calendar'].includes(next))setView(next)};
  function onInput(event){
    ...
    if(key==='accountBalance'){target.accountBalanceAsOf=today();const dateField=$('hf-'+parts[0]+'-'+parts[1]+'-accountBalanceAsOf');if(dateField)dateField.value=target.accountBalanceAsOf}
    if(parts[0]==='projects'){
  ...
  async function save(continueEditing=false){
    if(saving)return;if(!ready){retryNeeded=true;saveProblem('Le budget n’est pas encore chargé. Touche « Réessayer le chargement ».');return}if(loading){saveProblem('Attends la fin du chargement avant de sauvegarder.');return}let config;try{config=E.validate(draft);if(P)config=P.cleanPlan(config)}catch(error){saveProblem(error.message);return}
    ...
    try{const result=await api('PUT',body);if(stamp!==epoch)return;if(requestedPayments===paymentVersion)assetPayments=result.asset_payments||[];plan=P?P.merge(result.config,assetPayments):result.config;draft=structuredClone(plan);revision=result.revision;dirty=false;hpStability.complete(key);setView(continueEditing?'plan':'overview');status(continueEditing?'Plan enregistré. Tu peux choisir la prochaine dépense.':'Plan enregistré dans ton compte.');}
  ...
  async function onClick(event){
    ...
    if(action==='entry')return window.hpOpenBudgetEntry?.(['income','expense'].includes(b.dataset.entryType)?{entry_type:b.dataset.entryType}:undefined);
    if(action==='review-budget'){if(!draft.incomes.length){picker='income';setView('plan');renderEditor();$('hpFinanceTemplate')?.focus();return}setView('plan');$('hpFinanceReview')?.scrollIntoView?.({block:'center'});$('hf-reviewed')?.focus({preventScroll:true});return}
    ...
    if(['save','save-continue','add','remove','import','edit-project'].includes(action))closeScenario();
    if(action==='save-continue')return save(true);if(action==='save')return save();if(action==='retry'||action==='reload')return load();if(action==='replace')return load({force:true});if(action==='add')return add(b.dataset.list);
  ...
  function init(){ensure();load();window.addEventListener('hp-budget-loaded',()=>load());window.supabaseClient?.auth.onAuthStateChange((event,session)=>{if(event==='SIGNED_OUT'||(session?.user?.id&&owner&&owner!==session.user.id))reset();if(event==='SIGNED_IN')setTimeout(()=>load(),0)});document.addEventListener('click',event=>{if(event.target.closest('.tab')?.dataset.screen==='budget')setTimeout(()=>load(),0)});window.addEventListener('beforeunload',event=>{if(dirty){event.preventDefault();event.returnValue=''}})}
