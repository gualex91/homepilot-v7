(function(){
  ...
  function renderOverview(){
    ...
    if(!ready){host.innerHTML='<div class="finance-block">Ton budget personnel apparaîtra ici après chargement. Aucun montant n’est supposé nul.</div>';return}
    if(!r){host.innerHTML='<div class="finance-warning">Complète les lignes en cours dans « Mon plan » pour calculer les prévisions.</div>';return}
    const complete=r.complete,actual=r.actual,display=value=>complete?money(value):'À compléter';
    host.innerHTML=`${!complete?'<div class="finance-warning">Ton plan est incomplet. Ajoute tes montants et confirme que tu as vérifié tes postes de budget. <button type="button" class="alt" data-view="plan">Compléter mon plan</button></div>':''}<div class="finance-grid">${tile('Marge prévue après provisions',display(r.projectedMargin),'Revenus prévus − charges − dépenses courantes − réserves annuelles et projets. Ce n’est pas ton solde bancaire.',true,complete&&r.projectedMargin<0)}${tile('Revenus prévus',display(r.totals.income),'Selon les dates de versement du mois.')}${tile('Dépenses prévues',display(r.totals.bills+r.totals.flexible),'Charges datées + enveloppes courantes.')}${tile('À réserver ce mois',display(r.totals.provisions+r.totals.projects),'Provisions annuelles et projets ponctuels.')}${tile('Dépenses enregistrées',actual?money(actual.expense):'Indisponible','Opérations saisies, hors dates futures.')}</div><div class="finance-block"><h4>Avant ma prochaine rentrée d’argent</h4>${r.cash?`<div class="finance-stat primary"><span>Disponible estimé jusqu’au ${dateLabel(r.cash.nextPay)}, avant le versement</span><strong class="${r.cash.available<0?'finance-negative':''}">${money(r.cash.available)}</strong></div><p class="finance-note">Solde déclaré ${money(r.cash.balance)} − paiements ${money(r.cash.bills)} − dépenses courantes estimées ${money(r.cash.flexible)} − provisions ${money(r.cash.reserve)}.</p>`:'<p>À calculer : indique un solde vérifié aujourd’hui, une date de paie et un plan complet.</p><button type="button" class="alt" data-view="plan">Vérifier mon solde et mes dates</button>'}<p class="finance-note">Prévision à partir de demain. Les paiements du jour doivent déjà être déduits du solde déclaré. Les paiements à la date de paie sont déduits avant le versement. Aucun ajustement automatique des jours fériés ou de fin de semaine.</p></div><div class="finance-block"><h4>Prévu contre enregistré</h4><p class="finance-note">Plan actuel appliqué au mois sélectionné. Les opérations enregistrées ne constituent pas un relevé bancaire complet.</p>${actual?comparison(r):'<p role="alert">Les opérations n’ont pas toutes été chargées. Aucun total réel partiel n’est présenté.</p>'}</div><div class="finance-block"><h4>Mes prochaines actions</h4>${r.actions.map(a=>`<div class="finance-action"><b>${esc(a.title)}</b><p>${esc(a.body)}</p></div>`).join('')}</div>${projectCards(r)}<div class="finance-block"><h4>Mon coussin de sécurité</h4><p>${r.emergencyMonths!==null&&complete?`L’épargne de secours déclarée représente environ <b>${r.emergencyMonths.toLocaleString('fr-CA',{maximumFractionDigits:1})} mois</b> de dépenses marquées essentielles.`:'Ajoute ton épargne de secours et vérifie les dépenses marquées essentielles pour obtenir une estimation.'}</p><p class="finance-note">Cet indicateur décrit tes liquidités. Il ne détermine pas tes besoins d’assurance.</p></div><div class="finance-block"><h4>Préparer ma rencontre</h4><p>Organise tes questions sur ton budget, tes protections ou tes objectifs avec Guillaume-Alexandre Tremblay, conseiller en sécurité financière.</p><button type="button" data-action="summary">Afficher mon résumé</button><button type="button" class="alt" data-action="advisor">Consulter la fiche de Guillaume-Alexandre</button><p class="finance-note">Tes données restent privées. Aucun résumé n’est envoyé au conseiller. Les calculs sont éducatifs et ne remplacent pas une analyse personnalisée.</p><div id="hpFinanceSummary" hidden><label for="hpFinanceSummaryText">Résumé personnel — à réviser avant tout partage</label><textarea id="hpFinanceSummaryText" readonly></textarea><button type="button" class="alt" data-action="copy">Copier le résumé</button></div></div>`;
  }
  ...
  function editorRow(key,x,i){
    ...
    return `<div class="finance-edit-row">${input(p+'.label',income?'Source du revenu':annual?'Dépense annuelle':envelope?'Enveloppe':'Nom de la charge',x.label,'text','maxlength="120"')}${select(p+'.category','Catégorie des opérations correspondantes',x.category,categories)}${annual?`${input(p+'.annualAmount','Coût annuel total ($)',x.annualAmount,'number','min="0"')}${input(p+'.savedAmount','Déjà réservé hors solde disponible ($)',x.savedAmount,'number','min="0"')}${input(p+'.dueDate','Prochaine échéance',x.dueDate,'date')}`:`${input(p+'.amount',envelope?'Budget courant mensuel ($)':income?'Montant net par versement ($)':'Montant par paiement ($)',x.amount,'number','min="0"')}${!envelope?`${select(p+'.frequency','Fréquence',x.frequency,Object.entries(FREQ))}${input(p+'.anchorDate','Une date de versement ou paiement connue',x.anchorDate,'date')}<div ${x.frequency==='semimonthly'?'':'hidden'} data-second="${p}">${input(p+'.secondDay','Deuxième jour du mois (31 = dernier jour)',x.secondDay,'number','min="1" max="31" step="1"')}</div>`:''}`}${!income?checkbox(p+'.essential','Dépense essentielle',x.essential):''}<button type="button" class="finance-remove" data-action="remove" data-list="${key}" data-index="${i}">Retirer cette ligne</button></div>`;
  }
  ...
  function renderEditor(){
    ...
    const groups=[['incomes','1. Mes revenus nets','Salaire, prestations, travail autonome ou pension. Pour un revenu variable, utilise une estimation prudente et révise-la chaque mois.'],['bills','2. Mes charges régulières','Loyer, hypothèque, assurances, abonnements ou remboursement de prêt. N’ajoute pas ici une facture annuelle déjà prévue dans les provisions.'],['envelopes','3. Mes dépenses courantes','Épicerie, transport, sorties… Indique uniquement la portion variable, en plus des charges régulières de la même catégorie.'],['provisions','4. Mes dépenses annuelles','Taxes, immatriculations ou rentrée scolaire. Pour une dépense ponctuelle, utilise la section suivante.'],['projects','5. Entretiens et remplacements','Prépare le coût d’un projet ponctuel, sans renouvellement annuel automatique. Actualise les projets dont la date est passée.']];
    host.innerHTML=`<p class="finance-note">Les données saisies ici sont personnelles. Tu peux préparer ta part des dépenses d’un couple ou ton budget complet; aucun accès n’est accordé à un autre membre du foyer.</p>${!revision&&legacy&&(legacy.targets.length||legacy.bills.length)?'<div class="finance-warning">Des cibles ou rappels existent déjà. <button type="button" class="alt" data-action="import">Reprendre mes montants existants</button><p>À vérifier avant sauvegarde : les charges importées sont déduites des anciennes cibles pour calculer les enveloppes variables. Les dettes et hypothèques ne sont jamais additionnées automatiquement.</p></div>':''}${groups.map(([key,title,note],i)=>`<details ${i===0?'open':''}><summary>${title} · ${draft[key].length}</summary><p class="finance-note">${note}</p>${draft[key].map((x,j)=>editorRow(key,x,j)).join('')}<button type="button" class="alt" data-action="add" data-list="${key}">+ Ajouter ${key==='incomes'?'un revenu':key==='bills'?'une charge':key==='envelopes'?'une enveloppe':key==='projects'?'un projet':'une provision'}</button></details>`).join('')}<details><summary>6. Mon disponible et mon coussin</summary><p class="finance-note">Facultatif. Solde des comptes utilisés pour payer, hors épargne et provisions déjà réservées. Ce solde doit être revérifié chaque jour pour afficher une estimation à court terme.</p>${input('cash.balance','Solde utilisable déclaré ($)',draft.cash.balance,'number')}${input('cash.asOf','Date de vérification du solde',draft.cash.asOf,'date')}${checkbox('cash.todaySettled','Les paiements de cette journée sont déjà déduits de ce solde.',draft.cash.todaySettled)}${input('emergencySavings','Épargne disponible pour les imprévus ($)',draft.emergencySavings,'number','min="0"')}<p class="finance-note">Laisse vide si tu ne connais pas ce montant. Ne compte pas un immeuble, un véhicule ou une limite de crédit comme une épargne disponible.</p></details><div class="finance-warning">Enregistre chaque dépense une seule fois. Si les achats par carte sont déjà saisis, ne les compte pas de nouveau au remboursement de la carte. Les virements entre tes comptes ne sont pas des revenus.</div>${checkbox('reviewed','J’ai vérifié mes revenus, charges, dépenses courantes, provisions et projets. Les postes sans ligne ne s’appliquent pas à mon plan.',draft.reviewed)}<div class="finance-savebar"><p id="hpFinanceSaveStatus" class="finance-note" tabindex="-1" role="status" aria-live="polite"></p><button type="button" data-action="save" ${saving?'disabled':''}>${saving?'Enregistrement…':'Enregistrer mon plan'}</button><div class="finance-note" id="hpFinanceDirty">${dirty?'Modifications non enregistrées.':'Sauvegarde privée dans ton compte.'}</div></div>`;
    status(statusMessage,statusError);
  }
  ...
  async function save(){
    ...
    try{const result=await api('PUT',body);if(stamp!==epoch)return;plan=result.config;draft=structuredClone(plan);revision=result.revision;dirty=false;hpStability.complete(key);status('Plan enregistré dans ton compte.');}
