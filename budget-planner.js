(function(){
  ...
  function editorRow(key,x,i){
    ...
    const caption=amount===null?'Montant à compléter':money(E.cents(amount))+' · '+(annual?'par année':envelope?'par mois':FREQ[x.frequency]);
    return `<details class="finance-edit-row" data-budget-row="${p}"><summary><span id="hf-title-${key}-${i}">${esc(x.label||'Nouvelle ligne')}</span><small id="hf-caption-${key}-${i}">${esc(caption)}</small></summary>${input(p+'.label',income?'Nom du revenu':'Nom de la dépense',x.label,'text','maxlength="120"')}${annual?`${input(p+'.annualAmount','Montant à payer dans l’année ($)',x.annualAmount,'number','min="0"')}${input(p+'.dueDate','Prochaine date de paiement',x.dueDate,'date')}${input(p+'.savedAmount','Déjà mis de côté pour cette dépense ($)',x.savedAmount,'number','min="0"')}`:`${input(p+'.amount',envelope?'Total à prévoir par mois ($)':income?'Montant net reçu chaque fois ($)':'Montant payé chaque fois ($)',x.amount,'number','min="0"')}${!envelope?`${select(p+'.frequency','À quelle fréquence?',x.frequency,Object.entries(FREQ))}${input(p+'.anchorDate',income?'Une date où tu reçois ce revenu':'Une date où tu paies cette facture',x.anchorDate,'date')}<div ${x.frequency==='semimonthly'?'':'hidden'} data-second="${p}">${input(p+'.secondDay','Deuxième jour du mois (31 = dernier jour)',x.secondDay,'number','min="1" max="31" step="1"')}</div>`:'<p class="finance-note">Une estimation pour les achats du mois, sans date de paiement précise.</p>'}`}${x.category==='Épargne'?'<p class="finance-note">Cette mise de côté réduit ta marge prévue. Ne compte pas à nouveau un montant déjà prévu pour un projet ou une dépense annuelle.</p>':''}${x.category==='Revenus locatifs'?'<p class="finance-note">Si tu inscris le loyer reçu en entier, ajoute aussi les dépenses locatives. Si tu inscris un revenu net, ne déduis pas ces frais une deuxième fois.</p>':''}<details><summary>Catégorie et options</summary>${select(p+'.category','Catégorie',x.category,categories)}${!income?checkbox(p+'.essential','Cette dépense est essentielle pour moi.',x.essential):''}</details><button type="button" class="finance-remove" data-action="remove" data-list="${key}" data-index="${i}">Retirer cette ligne</button></details>`;
  }
  ...
  function onInput(event){
    ...
    const title=$('hf-title-'+parts[0]+'-'+parts[1]),caption=$('hf-caption-'+parts[0]+'-'+parts[1]);if(title)title.textContent=target.label||'Nouvelle ligne';if(caption){const value=parts[0]==='provisions'?target.annualAmount:target.amount;caption.textContent=value===null?'Montant à compléter':money(E.cents(value))+' · '+(parts[0]==='provisions'?'par année':parts[0]==='envelopes'?'par mois':FREQ[target.frequency])}
