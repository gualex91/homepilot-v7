(function(){
  ...
  function renderMoneyHeadline(){
    ...
    const info=I.analyze(draft,r),complete=r.complete;
    ...
    const note=complete?'Après tes dépenses et les sommes prévues à mettre de côté.':draft.incomes.length?'Les totaux saisis sont ci-dessous. Vérifie ton budget pour calculer ce qu’il reste.':'Ajoute tes revenus pour calculer ce qu’il reste.';
    host.innerHTML=`<p class="finance-money-label">Budget prévu · ${esc(period)} <span class="finance-budget-state">${dirty?'Modifications non enregistrées':complete?'Budget vérifié':'À vérifier'}</span></p><div class="finance-grid finance-balance">${tile(complete?(r.projectedMargin<0?'Il manque ce mois-ci':'Reste prévu ce mois-ci'):'Reste à calculer',complete?money(Math.abs(r.projectedMargin)):'—',note,true,complete&&r.projectedMargin<0)}</div><dl class="finance-equation"><div><dt>Revenus prévus</dt><dd>${money(r.totals.income)}</dd></div><div><dt>− Dépenses prévues</dt><dd>${money(expenses)}</dd></div><div><dt>− Épargne et réserves</dt><dd>${money(reserve)}</dd></div></dl><p class="finance-note">${complete?'Revenus − dépenses − épargne et réserves = reste prévu.':'Totaux des lignes saisies; ton budget est encore à vérifier.'} Ce n’est pas le solde de ton compte bancaire.</p>${!complete?`<button type="button" class="alt" data-action="review-budget">${draft.incomes.length?'Vérifier mon budget':'Ajouter mes revenus'}</button>`:''}`;
  }
