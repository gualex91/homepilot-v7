(function(){
  ...
  function renderMoneyHeadline(){
    ...
    host.innerHTML=`<p class="finance-money-label">Budget prévu · ${esc(period)} <span class="finance-budget-state">${dirty?'Modifications non enregistrées':complete?'Budget vérifié':canEstimate?'Estimation à vérifier':'Revenus prévus manquants'}</span></p><div class="finance-grid finance-balance">${tile(canEstimate?(r.projectedMargin<0?(complete?'Il manque ce mois-ci':'Manque estimé ce mois-ci'):(complete?'Reste prévu ce mois-ci':'Reste estimé ce mois-ci')):'Ajoute tes revenus prévus',canEstimate?money(Math.abs(r.projectedMargin)):'—',note,true,canEstimate&&r.projectedMargin<0)}</div><dl class="finance-equation"><div><dt>Revenus prévus</dt><dd>${money(r.totals.income)}</dd></div><div><dt>− Dépenses prévues</dt><dd>${money(expenses)}</dd></div><div><dt>− Épargne et réserves</dt><dd>${money(reserve)}</dd></div></dl><p class="finance-note">${complete?'Revenus − dépenses − épargne et réserves = reste prévu.':'Totaux des lignes saisies; ton budget est encore à vérifier.'} Ce n’est pas le solde de ton compte bancaire.</p>${!complete?`<button type="button" class="alt" data-action="review-budget">${draft.incomes.length?'Vérifier mon budget':'Ajouter mes revenus'}</button>`:''}`;
  }
