(function(){
  ...
  function expenseSuggestions(kind='expense'){
    const savings=kind==='savings',choices=EXPENSE_SUGGESTIONS.filter(([id])=>I.savingsCategory(C.templates.find(t=>t.id===id).category)===savings);
  ...
  function renderEditor(){
    ...
    const rows=(key,filter=()=>true)=>draft[key].map((x,i)=>activeExpense?.rowId===x.id||!filter(x)?'':editorRow(key,x,i)).join('');
