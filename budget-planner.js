(function(){
  ...
  function renderMoneyHeadline(){
    ...
    host.innerHTML=`<p class="finance-money-label">Budget prévu · ${esc(month)}${dirty?' · Modifications non enregistrées':''}</p><div class="finance-grid finance-balance">${tile(r.projectedMargin<0?'Ce qu’il manque ce mois-ci':'Ce qu’il reste ce mois-ci',display(Math.abs(r.projectedMargin)),marginMessage,true,complete&&r.projectedMargin<0)}${tile('Ce qui entre',display(r.totals.income),'Revenus nets prévus')}${tile('Ce qui sort / est réservé',display(info.outflow),'Dépenses + mises de côté')}</div><p class="finance-note">Prévision du mois sélectionné, pas ton solde bancaire. ${money(r.totals.provisions+r.totals.projects+info.savings)} sont prévus pour l’épargne et les réserves.</p>`;
  }
  ...
  function expenseSuggestions(){
    return `<div class="finance-suggestions"><h5>Dépenses fréquentes</h5><p class="finance-note">Choisis une dépense, puis inscris ton montant.</p><div class="finance-suggestion-grid">${EXPENSE_SUGGESTIONS.map(([id,label])=>{const t=C.templates.find(x=>x.id===id),index=existingSuggestion(t),active=activeExpense?.template===id,draftIndex=active?draft[t.key].findIndex(x=>x.id===activeExpense.rowId):-1,opened=active&&draftIndex>=0;return `<div class="finance-suggestion-item ${opened?'is-editing':''}"><button type="button" class="alt finance-suggestion" data-action="suggest-expense" data-template="${id}" aria-expanded="${opened}"><span>${esc(label)}</span><small>${opened?'Saisie en cours':index>=0?'Déjà ajoutée · modifier':'+ Ajouter un montant'}</small></button>${opened?`${editorRow(t.key,draft[t.key][draftIndex],draftIndex)}<p id="hpExpenseSaveStatus" class="finance-note" role="status" aria-live="polite"></p><button type="button" data-action="save-continue">Enregistrer et continuer</button>`:''}</div>`}).join('')}</div></div>`;
  }
