(function(){
 ...
 async function ensureFor(eq){const cl=c(),u=await user();if(!cl||!u)return;const rules=RULES[eq.equipment_type]||[];const {data:existing}=await cl.from('leisure_tasks').select('title').eq('user_id',u.id).eq('leisure_equipment_id',eq.id);const titles=new Set((existing||[]).map(x=>x.title));const rows=[];for(const [title,month,key] of rules){if(!titles.has(title))rows.push({user_id:u.id,leisure_equipment_id:eq.id,title,due_date:dateFor(month),status:'todo',diy_key:key,notes:'Créée automatiquement par HomePilot selon le type d’équipement.'})}
  if(eq.insurance_renewal_date&&!titles.has('Renouvellement assurance'))rows.push({user_id:u.id,leisure_equipment_id:eq.id,title:'Renouvellement assurance',due_date:eq.insurance_renewal_date,status:'todo',notes:'Rappel annuel HomePilot'});
  if(eq.registration_renewal_date&&!titles.has('Renouvellement immatriculation'))rows.push({user_id:u.id,leisure_equipment_id:eq.id,title:'Renouvellement immatriculation',due_date:eq.registration_renewal_date,status:'todo',notes:'Rappel annuel HomePilot'});
