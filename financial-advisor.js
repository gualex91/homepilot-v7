(function(){
 ...
 function ensureModal(){if($('hpAdvisorModal'))return;const d=document.createElement('div');d.id='hpAdvisorModal';d.className='hidden';d.style='position:fixed;inset:0;z-index:180;background:#0009;padding:18px;overflow:auto';d.innerHTML='<div class="card" style="max-width:620px;margin:3vh auto"><div class="row"><h2>Conseiller financier</h2><button class="alt" onclick="hpCloseAdvisor()">Fermer</button></div><div id="hpAdvisorBody"></div></div>';document.body.appendChild(d)}
 window.hpCloseAdvisor=()=>$('hpAdvisorModal')?.classList.add('hidden');
 window.hpFindAdvisor=()=>{ensureModal();$('hpAdvisorBody').innerHTML=`<div class="card"><div class="row"><div><b>${PROFILE.name}</b><div class="muted">${PROFILE.title}</div></div><span class="pill">Conseiller HomePilot</span></div><p class="muted">${PROFILE.firm}</p><p>Disponible comme référence financière HomePilot partout au Québec.</p><div class="notice"><b>Services</b><br>${PROFILE.services.join(' • ')}</div><p style="margin-top:14px"><a href="${PROFILE.profileUrl}" target="_blank" rel="noopener">Voir la fiche officielle iA et contacter Guillaume-Alexandre</a></p></div>`;$('hpAdvisorModal').classList.remove('hidden')};
 function addBudgetButton(){const s=$('budget');if(!s||s.querySelector('.hpFindAdvisorBtn'))return;const card=document.createElement('div');card.className='card';card.innerHTML='<div class="row"><div><div class="badge">Accompagnement</div><h3 style="margin:8px 0 4px">Besoin d’un conseiller?</h3><p class="muted" style="margin:0">Assurance, épargne, retraite et stratégies financières.</p></div></div><button class="hpFindAdvisorBtn" style="width:100%;margin-top:12px" onclick="hpFindAdvisor()">👤 Trouver un conseiller</button>';s.appendChild(card)}
