(function(){
 ...
 function hardenShow(){if(window.__hpShowHardened||typeof window.show!=='function')return;window.__hpShowHardened=true;const orig=window.show;window.show=function(id){const target=$(id);if(!target){console.warn('Nuvabri: écran introuvable',id);return}try{return orig.apply(this,arguments)}catch(e){console.error('Nuvabri navigation error',e);document.querySelectorAll('.screen').forEach(x=>x.classList.remove('on'));target.classList.add('on')}}}
 ...
 function init(){run();let t;new MutationObserver(()=>{clearTimeout(t);t=setTimeout(run,80)}).observe(document.body,{childList:true,subtree:true});window.addEventListener('error',e=>console.error('Nuvabri UI error:',e.error||e.message));window.addEventListener('unhandledrejection',e=>console.error('Nuvabri async error:',e.reason));setTimeout(run,2500)}
