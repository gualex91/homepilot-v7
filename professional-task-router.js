(function(){
  ...
  async function fetchWithTimeout(url,opts,ms=6000){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),ms);try{return await fetch(url,{...opts,signal:controller.signal})}catch(e){if(e?.name==='AbortError')throw new Error('La recherche n’a pas répondu assez vite. Réessaie.');throw e}finally{clearTimeout(timer)}}
  function cacheKey(route,prop){return 'hp-pro:'+norm(route.service||route.category)+':'+norm(prop.city||'')+':'+norm(prop.postal_code||'')}
  function readCache(key){try{const x=JSON.parse(sessionStorage.getItem(key)||'null');if(x&&Date.now()-x.at<300000)return x.rows||[]}catch(e){}return null}
  function writeCache(key,rows){try{sessionStorage.setItem(key,JSON.stringify({at:Date.now(),rows}))}catch(e){}}
  function renderRows(body,rows,route,prop){if(!rows.length){body.innerHTML=`<h3>Aucun professionnel confirmé pour ${esc(route.label)}</h3><p class="muted">HomePilot préfère afficher zéro résultat plutôt qu’une entreprise dont ce service précis n’est pas confirmé.</p><p class="muted">Ville utilisée : ${esc(prop.city||'non définie')}.</p>`;return}body.innerHTML=`<p class="muted">Résultats correspondant précisément à <b>${esc(route.label)}</b> à ${esc(prop.city||'ta région')}.</p>`+rows.map(p=>`<div class="card"><div class="row"><b>${esc(p.business_name)}</b>${(p.service_categories||[]).includes(route.service)?'<span class="badge">Service confirmé</span>':''}</div>${p.description?`<p class="muted">${esc(p.description)}</p>`:''}${p.phone?`<p><a href="tel:${esc(p.phone)}">${esc(p.phone)}</a></p>`:''}${p.website?`<p><a href="${esc(p.website)}" target="_blank" rel="noopener">Voir le site</a></p>`:''}</div>`).join('')}
  async function searchPros(text){const m=modal(),body=$('hpProBody'),prop=currentProperty(),route=routeFor(text);m.classList.remove('hidden');if(!prop){body.innerHTML='<div class="notice">Sélectionne d’abord une propriété pour utiliser sa ville.</div>';return}const key=cacheKey(route,prop),cached=readCache(key);if(cached){renderRows(body,cached,route,prop);return}body.innerHTML='<p class="muted">Recherche…</p>';
    try{const t=token();if(!t)throw new Error('Session expirée. Reconnecte-toi.');const qs=new URLSearchParams({category:route.category,city:prop.city||'',postal:prop.postal_code||'',region:prop.region||prop.administrative_region||''});if(route.service)qs.set('service',route.service);const r=await fetchWithTimeout('/api/professionals?'+qs.toString(),{headers:{Authorization:'Bearer '+t},cache:'no-store'},6000);const data=await r.json().catch(()=>null);if(!r.ok)throw new Error(data?.error||'Impossible de charger le répertoire.');const rows=data?.rows||[];writeCache(key,rows);renderRows(body,rows,route,prop)}catch(e){console.error('professional search',e);body.innerHTML='<div class="notice">'+esc(e?.message||'Impossible de charger le répertoire.')+'</div><button class="alt" id="hpRetryPros" style="width:100%;margin-top:10px">Réessayer</button>';const retry=$('hpRetryPros');if(retry)retry.onclick=()=>searchPros(text)}}
  ...
  window.hpOpenPros=searchPros;window.hpFindLeisurePro=key=>searchPros(key||'loisir');window.hpProfessionalRouteForTask=routeFor;
})();
