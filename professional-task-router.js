(function(){
  ...
  async function fetchWithTimeout(url,opts,ms=12000){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),ms);try{return await fetch(url,{...opts,signal:controller.signal})}catch(e){if(e?.name==='AbortError')throw new Error('La recherche prend trop de temps. Réessaie dans quelques secondes.');throw e}finally{clearTimeout(timer)}}
  async function searchPros(text){const m=modal(),body=$('hpProBody'),prop=currentProperty(),route=routeFor(text);m.classList.remove('hidden');body.innerHTML='<p class="muted">Recherche par service précis…</p>';if(!prop){body.innerHTML='<div class="notice">Sélectionne d’abord une propriété pour utiliser sa ville.</div>';return}
    ...
    }catch(e){console.error('professional search',e);body.innerHTML='<div class="notice">'+esc(e?.message||'Impossible de charger le répertoire.')+'</div><button class="alt" id="hpRetryPros" style="width:100%;margin-top:10px">Réessayer</button>';const retry=$('hpRetryPros');if(retry)retry.onclick=()=>searchPros(text)}}
