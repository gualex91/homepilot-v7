(function(){
  ...
  const TASK_ROUTES=[
    ...
    [['echangeur d air','conduit de ventilation','conduits de ventilation'],'hvac','ventilation_cleaning','ventilation / échangeur d’air',true],
    [['chauffe-eau','chauffe eau','pompe de puisard','plomberie','fuite d eau','fuite eau','robinet','toilette','lavabo','drain','egout','debouchage'],'plumbing','plumbing','plomberie',false],
    [['ramonage','cheminee'],'chimney','chimney','cheminée / ramonage',false],
    [['fosse septique','installation septique','champ d epuration'],'septic','septic','service septique',false],
    [['electricite','electrique','prise','panneau electrique','detecteur de fumee','monoxyde de carbone'],'electrical','electrical','électricité',false],
    ...
    [['gouttiere','toiture','toit','bardeau'],'roofing','','toiture / gouttières',false],
    [['porte de garage','garage door'],'garage_door','','porte de garage',false],
    [['deneigement'],'snow_removal','','déneigement',false],
    [['tonte de pelouse','pelouse','gazon','arbre','elagage','abattage'],'landscaping','','aménagement paysager',false]
  ];
  ...
  function routeFor(text){
    ...
    if(t.includes('thermopompe')){
      if(t.includes('install')) return {category:'hvac',service:'heat_pump_installation',label:'installation de thermopompe',strict:true};
      if(t.includes('nettoy')||t.includes('filtre')) return {category:'hvac',service:'heat_pump_cleaning',label:'nettoyage de thermopompe',strict:true};
      if(t.includes('inspection')||t.includes('saisonn')||t.includes('entretien')||t.includes('verif')||t.includes('repar')) return {category:'hvac',service:'heat_pump_maintenance',label:'inspection / entretien de thermopompe',strict:true};
      return {category:'hvac',service:'heat_pump_maintenance',label:'service de thermopompe',strict:true};
    }
    for(const [keys,category,service,label,strict] of TASK_ROUTES){
      if(keys.some(k=>t.includes(norm(k)))) return {category,service,label,strict};
    }
    return {unknown:true,label:'ce service',strict:true};
  }
  ...
  async function searchWithRoute(route){
    const m=modal(),body=$('hpProBody'),prop=currentProperty();m.classList.remove('hidden');
    if(route.unknown){body.innerHTML='<div class="notice"><b>Service non associé.</b><br>HomePilot ne propose aucun professionnel pour cette tâche tant que le métier précis n’est pas identifié.</div>';return}
    if(!prop){body.innerHTML='<div class="notice">Sélectionne d’abord une propriété pour utiliser sa ville.</div>';return}
    body.innerHTML='<p class="muted">Recherche…</p>';
    try{
      const t=token();if(!t)throw new Error('Session expirée. Reconnecte-toi.');
      const qs=new URLSearchParams({category:route.category,city:prop.city||'',postal:prop.postal_code||'',region:prop.region||prop.administrative_region||'',v:'3'});if(route.service)qs.set('service',route.service);
      const ck=qs.toString(),hit=cache.get(ck);let data;
      if(hit&&hit.data?.rows?.length&&Date.now()-hit.at<300000)data=hit.data;
      else{const r=await fetchFast('/api/professionals?'+ck,{headers:{Authorization:'Bearer '+t},cache:'no-store'},6000);data=await r.json().catch(()=>null);if(!r.ok)throw new Error(data?.error||'Impossible de charger le répertoire.');if(data?.rows?.length)cache.set(ck,{at:Date.now(),data});else cache.delete(ck)}
      const rows=data?.rows||[];
      if(!rows.length){body.innerHTML=`<h3>Aucun professionnel confirmé pour ${esc(route.label)}</h3><p class="muted">Ville utilisée : ${esc(prop.city||'non définie')}.</p>`;return}
