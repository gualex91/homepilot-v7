(function(){
  ...
  function routeFor(text){
    const t=norm(text);
    if(t.includes('thermopompe')){
      if(t.includes('install'))return {category:'hvac',service:'heat_pump_installation',label:'installation de thermopompe'};
      if(t.includes('nettoy')||t.includes('filtre'))return {category:'hvac',service:'heat_pump_cleaning',label:'nettoyage de thermopompe'};
      return {category:'hvac',service:'heat_pump_maintenance',label:'inspection / entretien de thermopompe'};
    }
    for(const [keys,category,service,label] of TASK_ROUTES)if(keys.some(k=>t.includes(norm(k))))return {category,service,label};
    return {unknown:true,label:'ce service'};
  }
  ...
  async function searchPros(text){
    const route=routeFor(text),m=modal(),body=$('hpProBody'),prop=currentProperty();m.classList.remove('hidden');
    if(route.unknown){body.innerHTML='<div class="notice"><b>Service non associé.</b><br>HomePilot ne propose rien au hasard.</div>';return}
    if(!prop){body.innerHTML='<div class="notice">Sélectionne d’abord une propriété.</div>';return}
    body.innerHTML='<p class="muted">Recherche…</p>';
    try{const t=token();if(!t)throw new Error('Session expirée. Reconnecte-toi.');const qs=new URLSearchParams({category:route.category,city:prop.city||'',postal:prop.postal_code||'',region:prop.region||prop.administrative_region||'',v:'4'});if(route.service)qs.set('service',route.service);const r=await fetchFast('/api/professionals?'+qs,{headers:{Authorization:'Bearer '+t},cache:'no-store'});const data=await r.json().catch(()=>null);if(!r.ok)throw new Error(data?.error||'Impossible de charger le répertoire.');const rows=data?.rows||[];body.innerHTML=rows.length?`<p class="muted">Résultats pour <b>${esc(route.label)}</b> à ${esc(prop.city||'ta région')}.</p>`+rows.map(p=>`<div class="card"><b>${esc(p.business_name)}</b>${p.phone?`<p><a href="tel:${esc(p.phone)}">${esc(p.phone)}</a></p>`:''}${p.website?`<p><a href="${esc(p.website)}" target="_blank" rel="noopener">Voir le site</a></p>`:''}</div>`).join(''):`<h3>Aucun professionnel confirmé pour ${esc(route.label)}</h3>`}catch(e){body.innerHTML='<div class="notice">'+esc(e?.message||'Impossible de charger le répertoire.')+'</div>'}
  }
  async function searchLeisure(key){
    const route=leisureRoute(key),m=modal(),body=$('hpProBody'),prop=currentProperty();m.classList.remove('hidden');
    if(!route){body.innerHTML='<div class="notice"><b>Catégorie loisir non reconnue.</b></div>';return}
    if(!prop){body.innerHTML='<div class="notice">Sélectionne d’abord une propriété pour utiliser sa région.</div>';return}
    body.innerHTML='<p class="muted">Recherche…</p>';
    try{const t=token();if(!t)throw new Error('Session expirée. Reconnecte-toi.');const qs=new URLSearchParams({category:route.category,city:prop.city||'',region:prop.region||prop.administrative_region||'',v:'1'});const r=await fetchFast('/api/leisure-professionals?'+qs,{headers:{Authorization:'Bearer '+t},cache:'no-store'});const data=await r.json().catch(()=>null);if(!r.ok)throw new Error(data?.error||'Impossible de charger le répertoire loisirs.');const rows=data?.rows||[];body.innerHTML=rows.length?`<p class="muted">Spécialistes <b>${esc(route.label)}</b> près de ${esc(prop.city||'ta région')}.</p>`+rows.map(p=>`<div class="card"><div class="row"><b>${esc(p.business_name)}</b>${p.verified?'<span class="badge">Vérifié</span>':''}</div><div class="muted">${esc(p.city||'')}${p.region?' • '+esc(p.region):''}</div>${p.services?.length?`<p class="muted">${esc(p.services.join(' • '))}</p>`:''}${p.phone?`<p><a href="tel:${esc(p.phone)}">${esc(p.phone)}</a></p>`:''}${p.website?`<p><a href="${esc(p.website)}" target="_blank" rel="noopener">Voir le site</a></p>`:''}</div>`).join(''):`<div class="notice"><b>Aucun spécialiste ${esc(route.label)} trouvé dans ta région.</b></div>`}catch(e){body.innerHTML='<div class="notice">'+esc(e?.message||'Impossible de charger le répertoire loisirs.')+'</div>'}
  }
  document.addEventListener('click',e=>{const b=e.target?.closest?.('button');if(!b)return;if(!(b.classList.contains('hpFindPro')||/Trouver un professionnel/i.test(b.textContent||'')))return;const oc=b.getAttribute('onclick')||'';const lm=oc.match(/hpFindLeisurePro\(['\"]([^'\"]+)['\"]\)/);e.preventDefault();e.stopImmediatePropagation();if(lm)return searchLeisure(lm[1]);return searchPros((b.closest('.notice,.card,li')||b).textContent||'')},true);
